...
def make_my_data(percapita):
    ...
    for country in list(percapita.columns):
        ...
        start = datetime(2020, 4, 4, 12, 30)
        end = datetime(2020, 11, 4, 12, 30)
        my_dict = dict()
...
# 매개변수 o가 날짜형이면
def my_converter(o):
    if isinstance(o, datetime):  # 매개변수 o로 전달된 값이 datetime으로부터 만들어진 인스턴스에 해당하면
        return o.__str__()   # str 문자열로 변환해서 반환해라


def covid_dump():
    df = load_data()
    df, countries = select_countries(df) # 두 개의 변수 반환받음
    df = sum_cases(df)
    covid = reshape(df)
